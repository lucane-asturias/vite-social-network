...
<template>
  <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4">
    <div class="main-left col-span-3 space-y-4">
      ...
      <!-- Search Bar -->
      <div class="bg-white border border-gray-200 rounded-lg md:-my-3">
        <vee-form @submit="onSearchQuery" class="p-4 flex space-x-4">
          <vee-field type="search" name="query" 
            class="p-4 w-full bg-gray-100 rounded-lg" placeholder="What are you looking for?" 
          />
          <ErrorMessage class="text-lg text-red-500 mb-2" name="query" />
      ...
      <!-- Posts Section -->
      <div v-for="post in posts" :key="post.id" class="p-4 bg-white border border-gray-200 rounded-lg mt-5">
        <FeedItem :post="post" />   
      </div>

    </div>

    <div class="main-right md:col-span-1 space-y-4 mt-5">
      <PeopleYouMayKnow />
      <Trends />
    </div>

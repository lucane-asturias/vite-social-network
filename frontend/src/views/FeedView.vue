<script lang="ts" setup>
  ...
  const feedSchema = reactive({
    body: 'required|min:3|max:255|alpha_spaces'
  })

  const handleForm = async (values, { resetForm }) => {
    try {
      const { data } = await axios.post<PostsType>('/api/posts/create/', {
        'body': values.body
      })
  ...
  <div class="max-w-7xl mx-auto grid grid-cols-4 gap-4">
    ...
    <div class="main-center col-span-2 space-y-4">
      <div class="bg-white border border-gray-200 rounded-lg">
        <vee-form :validation-schema="feedSchema" @submit="handleForm">
